...
// Dans cette fonction, la valeur que l'on transmet en e.target.value est l'id
const DropdownMenu = ({ onChange, donneesMap, valeurDefaut, largeur, margeGauche }) => {
    ...
    let optionItems = donneesMap.map((donnee => 
        <option key={donnee.id} value={donnee.id}>{donnee.champs}</option>
    ));
